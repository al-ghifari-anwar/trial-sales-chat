...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($rowSuratJalan == null) {
        ...
        foreach ($rowSuratJalan as $rowSuratJalan) {
            ...
            $getDefaultPotongan = mysqli_query($conn, "SELECT SUM(qty_produk) AS jmlItem, potongan_cod AS total_potongan FROM tb_detail_surat_jalan JOIN tb_produk ON tb_produk.id_produk = tb_detail_surat_jalan.id_produk JOIN tb_master_produk ON tb_master_produk.id_master_produk = tb_produk.id_master_produk WHERE id_surat_jalan = '$id_surat_jalan' AND is_bonus = 0 AND tb_master_produk.is_default_potongan = 1 GROUP BY id_surat_jalan");

            $rowDefaultPotongan = $getDefaultPotongan->fetch_array(MYSQLI_ASSOC);
            ...
            $potonganCrossSelling = 0;
            if ($rowDefaultPotongan['jmlItem'] > 0) {
                $potonganCrossSelling = $rowNotDefaultPotongan['total_potongan'] * $rowNotDefaultPotongan['jmlItem'];
            }
            ...
            if ($rowCheckInv) {
                ...
                $date_jatem = date('Y-m-d H:i:s', strtotime("+" . $invArray['termin_payment'] . " days", strtotime($date_invoice)));
