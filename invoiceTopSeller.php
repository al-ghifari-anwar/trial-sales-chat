...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($_POST['id_surat_jalan'])) {
        ...
        if ($rowSuratJalan == null) {
            ...
            $id_apporder = $rowSuratJalan['id_apporder'];
            $resultApporder = mysqli_query($conn, "SELECT * FROM tb_apporder WHERE id_apporder = '$id_apporder'");
            $rowApporder = $resultApporder->fetch_array(MYSQLI_ASSOC);
            ...
            $subtotal = $rowSubTotals['subtotal'];
            $discount_app_invoice = $rowApporder['discount_apporder'];
            ...
            if ($subtotal_invoice <= 0) {
                ...
                $total_invoice = $subtotal_invoice - $rowApporder['discount_apporder'];
            }
            ...
            if ($rowCheckInv) {
                ...
                $resultInvoice = mysqli_query($conn, "INSERT INTO tb_invoice(id_surat_jalan,no_invoice,date_invoice,bill_to_name,bill_to_address,bill_to_phone,subtotal_invoice,discount_app_invoice,total_invoice) VALUES($id_surat_jalan, '$no_invoice', '$date_invoice', '$bill_to_name', '$bill_to_address', '$bill_to_phone', $subtotal_invoice,$discount_app_invoice, $total_invoice)");

                if ($resultInvoice) {
                    $id_invoice = mysqli_insert_id($conn);
                    if ($_SERVER['SERVER_NAME'] == 'saleswa.topmortarindonesia.com') {
                        // Send Notif
                        $curl = curl_init();
