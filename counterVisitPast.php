...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    // Target Calculation
    $date1 = new DateTime(date('Y-m-d', strtotime($year . "-" . $month . "-01")));
    $date2 = new DateTime(date('Y-m-t', strtotime($year . "-" . $month . "-01")));
    $date2->modify('+1 day');
    ...
    foreach ($period as $date) {
        ...
        $totalConfirmed += count($arrayTotal) < $checkYes['total_confirmed'] ? count($arrayTotal) : $checkYes['total_confirmed'];
