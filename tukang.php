...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['id'])) {
        ...
        if (isset($_GET['c'])) {
            ...
            $result = mysqli_query($conn, "SELECT * FROM tb_tukang JOIN tb_skill ON tb_skill.id_skill = tb_tukang.id_skill WHERE (nama LIKE '%$key%' OR nomorhp LIKE '%$key%') AND tb_tukang.id_city = '$id_city' ORDER BY tb_tukang.created_at DESC");
        } else {
            $key = isset($_GET['key']) ? $_GET['key'] : '';
            $id_dist = $_GET['dst'];
            $result = mysqli_query($conn, "SELECT * FROM tb_tukang JOIN tb_skill ON tb_skill.id_skill = tb_tukang.id_skill JOIN tb_city ON tb_city.id_city = tb_tukang.id_city WHERE (nama LIKE '%$key%' OR nomorhp LIKE '%$key%') AND id_distributor = '$id_dist' ORDER BY tb_tukang.created_at DESC");
        }
