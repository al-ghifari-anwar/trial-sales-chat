        ...
        if ($tagih_mingguan == 1) {
            if ($days > "-28" && $days < "30") {
                if ($days % 7 == 0) {
                    ...
                    if ($sisaHutang > 0) {
                        ...
                        if ($renvisArray == null) {
                            ...
                            if ($renvisJatemArray == null) {
                                ...
                                if ($insertRenvis) {
                                    $response = ["response" => 200, "status" => "ok", "message" => "Berhasil menyimpan data rencana visit!", "days" => $days, "no_inv" => $invArray['no_invoice'] . "-id-" . $invArray['id_contact']];
                    ...
                    $response = ["message" => "Blm seminggu", "days" => $days, "no_inv" => "INV:" . $invArray['no_invoice']];
                    echo json_encode($response);
                }
            } else {
                $response = ["message" => "Sudah jatem", "days" => $days, "no_inv" => "INV:" . $invArray['no_invoice']];
                echo json_encode($response);
            }
        } else {
