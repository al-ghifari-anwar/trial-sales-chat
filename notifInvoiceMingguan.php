    ...
    foreach ($invArray as $invArray) {
        ...
        if (date('Y-m-d') > date('Y-m-d', strtotime("+" . $invArray['termin_payment'] . " days", strtotime($invArray['date_invoice'])))) {
            if ($days % 7 == 0) {
                ...
                if ($sisaHutang > 0) {
                    $response = ["message" => "Sudah waktunya", "days" => $days, "date_inv" => $invArray['date_invoice'], "toko" => $nama, "id_inv" => $id_invoice];
                ...
                $response = ["message" => "Belum waktunya", "days" => $days, "date_inv" => $invArray['date_invoice'], "toko" => $nama, "id_inv" => $id_invoice];
                echo json_encode($response);
            }
            // echo $jatuhTempo;
        }
