...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if ($_GET['p'] == 1) {
        ...
        $resultStore = mysqli_query($conn, "SELECT * FROM tb_contact JOIN tb_surat_jalan ON tb_surat_jalan.id_contact = tb_contact.id_contact WHERE tb_surat_jalan.id_courier = '$id_courier' AND tb_surat_jalan.is_closing = 0");
        ...
        while ($row = $resultDetail->fetch_array(MYSQLI_ASSOC)) {
            ...
            $getStokOut = mysqli_query($conn, "SELECT SUM(qty_produk) AS jml_stokOut FROM tb_detail_surat_jalan JOIN tb_produk ON tb_produk.id_produk = tb_detail_surat_jalan.id_produk JOIN tb_surat_jalan ON tb_surat_jalan.id_surat_jalan = tb_detail_surat_jalan.id_surat_jalan JOIN tb_master_produk ON tb_master_produk.id_master_produk = tb_produk.id_master_produk WHERE tb_produk.id_city IN (SELECT id_city FROM tb_city WHERE id_gudang_stok = '$id_gudang_stok') ")->fetch_array(MYSQLI_ASSOC);
